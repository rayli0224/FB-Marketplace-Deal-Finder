...
def process_single_listing(
    ...
    logger.debug(f"Processing listing: '{listing.title}' (${listing.price:.2f})")
    
    query_result = generate_ebay_query_for_listing(listing, original_query)
    if not query_result:
        logger.debug(f"Skipping listing '{listing.title}': Failed to generate eBay query")
        return None
    ...
    if not ebay_stats:
        logger.debug(f"Skipping listing '{listing.title}': No eBay stats found")
        return None
    ...
    if deal_score is None:
        logger.debug(f"Skipping listing '{listing.title}': Could not calculate deal score")
        return None
    
    logger.debug(f"Listing '{listing.title}': Deal score = {deal_score}% (eBay avg: ${ebay_stats.average:.2f})")
    
    # Step 4: Check if meets threshold
    if deal_score < threshold:
        logger.debug(f"Skipping listing '{listing.title}': Deal score {deal_score}% below threshold {threshold}%")
        return None
